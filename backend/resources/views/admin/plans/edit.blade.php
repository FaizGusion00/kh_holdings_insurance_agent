...
<div class="flex justify-between items-center mb-6">
	<h1 class="text-2xl font-semibold">Edit Plan: {{ $plan->name }}</h1>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
			...
			<div class="md:col-span-2">
				...
				<textarea name="description" rows="3" 
					class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $plan->description) }}</textarea>
				@error('description')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
			</div>

			<div>
				<label class="block text-sm font-medium text-gray-700 mb-2">Price (cents)</label>
				<input type="number" name="price_cents" value="{{ old('price_cents', $plan->price_cents) }}" min="0"
					class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
			...
			<div>
				...
				<select name="uses_percentage_commission" 
					...
					<option value="1" {{ old('uses_percentage_commission', $plan->uses_percentage_commission) == '1' ? 'selected' : '' }}>Percentage</option>
					<option value="0" {{ old('uses_percentage_commission', $plan->uses_percentage_commission) == '0' ? 'selected' : '' }}>Fixed Amount</option>
				</select>
