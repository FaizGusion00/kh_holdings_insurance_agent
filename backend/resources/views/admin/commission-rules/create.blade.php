...
<div class="space-y-6">
        ...
        <div class="mx-auto max-w-7xl">
            <div class="px-4 py-6 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold tracking-tight text-gray-900">Create Commission Rule</h1>
                        <p class="mt-2 text-sm text-gray-700">Add a new commission rule for agents.</p>
                    </div>
    ...
    <div class="bg-white shadow rounded-lg fade-in" style="animation-delay: 0.1s;">
        <div class="px-4 py-5 sm:p-6">
            <form action="{{ route('admin.commission-rules.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="plan_type" id="plan_type" required
                            ...
                            <option value="senior_care" {{ old('plan_type') == 'senior_care' ? 'selected' : '' }}>Senior Care</option>
                            <option value="medical_card" {{ old('plan_type') == 'medical_card' ? 'selected' : '' }}>Medical Card</option>
                        </select>
                        @error('plan_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="tier_level" id="tier_level" required
                            ...
                            <option value="1" {{ old('tier_level') == '1' ? 'selected' : '' }}>Tier 1</option>
                            <option value="2" {{ old('tier_level') == '2' ? 'selected' : '' }}>Tier 2</option>
                            <option value="3" {{ old('tier_level') == '3' ? 'selected' : '' }}>Tier 3</option>
                            <option value="4" {{ old('tier_level') == '4' ? 'selected' : '' }}>Tier 4</option>
                            <option value="5" {{ old('tier_level') == '5' ? 'selected' : '' }}>Tier 5</option>
                        </select>
                        @error('tier_level')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="commission_percentage" id="commission_percentage" value="{{ old('commission_percentage') }}" step="0.01" min="0" max="100" required
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
