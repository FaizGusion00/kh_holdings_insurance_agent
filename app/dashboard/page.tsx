...
function StatCard({ title, value, highlight, icon: Icon, trend }: { 
	...
	return (
		<div className={`rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 transition-all duration-300 hover:shadow-lg ${
			highlight 
			...
			<div className="flex items-start justify-between">
				<div className="flex-1">
					<div className="flex items-center gap-2 mb-2">
						{Icon && <Icon className="w-4 h-4 text-blue-600 opacity-70" />}
					...
					{trend && (
						<div className="flex items-center gap-1 mt-2">
							<ArrowUpRight className={`w-3 h-3 ${trend.isPositive ? 'text-green-500' : 'text-red-500'}`} />
				...
				{title === "Total Member" && (
					<div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-blue-100 flex items-center justify-center">
						<CircleHelp className="text-blue-600 w-4 h-4 sm:w-5 sm:h-5" />
					</div>
				)}
			</div>
		</div>
	...
	return (
		<PageTransition>
			<div className="min-h-screen flex items-center justify-center p-2 sm:p-3 md:p-4 lg:p-6 bg-gradient-to-br from-blue-50/30 via-white to-emerald-50/30">
				<div className="relative w-full max-w-5xl xl:max-w-6xl green-gradient-border p-3 sm:p-4 md:p-6 lg:p-8 xl:p-10">
					{/* Profile Badge */}
					<FadeIn delay={0.3}>
					...
					<div className="flex flex-col gap-5 sm:gap-6 md:gap-8">
						...
						<FadeIn delay={0.4}>
							<div className="text-center sm:text-left">
								...
								<p className="text-gray-500 text-sm sm:text-base">Hello, welcome back!</p>
								<h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold leading-tight bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">
									NOR ZAKIAH BINTI WAN OMAR
								</h1>
							</div>
						</FadeIn>
						
						<div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] xl:grid-cols-[400px_1fr] gap-4 sm:gap-6">
								...
								<div className="flex flex-col gap-3 sm:gap-4">
									<div className="flex items-center justify-between">
										<div>
											...
											<div className="h-1 w-16 sm:w-20 bg-gradient-to-r from-blue-600 to-emerald-500 rounded-full" />
										</div>
										<button 
											onClick={()=> setShowAdd(true)} 
											className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-gradient-to-r from-blue-600 to-emerald-600 text-white grid place-content-center hover:from-blue-700 hover:to-emerald-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
										>
